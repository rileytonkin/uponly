...
- Under the title: the value in dollars and cents, then the change over the chart's range as a green or red pill with what it's measured against ("↑ 21.0% vs $10,000.00 prev 1M", measured from the first saved value when the history is shorter than the range; hover for the dates), and **All-time** (profit against what you paid, for holdings with recorded purchases). A **1W 1M 3M 1Y All** control sits above the chart and sets every change on the page; the line is green when the range ends at or above where it started and red when below. On All assets, each row shows its value and its change over the range and opens its page. In privacy mode amounts become dots but percentages stay, since they don't say how much you hold. Income & spending keeps its By month, By year or All time selector.
