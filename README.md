...
Up Only starts empty. Three short setup steps save your recovery code, choose what to track, and offer optional public prices and exchange rates. Finish with **Add your info** or **Do this later**. Add your own bank accounts, crypto portfolios, income and expenses inside the app. No account registration, analytics, connected wallets or cloud sync.

## What it does

- One top-left Monthly, Annual or All time selector controls both Performance and Net worth. Performance can show All, Personal, or a connected company. Net worth values balances at the selected period end.
...
- A conditional Needs attention button opens missing balances, quantities, prices, accounting and personal spending reviews. Add custom accounts, balances, crypto, metals or statements directly from that page. A balance or partial import never confirms complete spending; closed months require an explicit review, and new accounts or changed entries reopen the relevant reviews.
...
[Preev](https://preev.com/) describes WebSocket streams from 12 exchanges and a 24-hour volume-weighted average. It is useful as a reference for a future live-price display; this release uses documented provider APIs and 15-minute refreshes. It does not scrape Preev or claim continuous live ticks. Daily catch-up supplies the app's daily charts, not every trade missed while offline.

## Add your info

Manage → **Add your info** accepts multiple CSV files, dropped files, and cells copied from Excel or Numbers. Choose **Statements**, **Bank balances**, **Crypto holdings**, or **Precious metals**. Each follows Input → Review → Save. Nothing is saved until the entire accepted batch validates; locking clears unsaved drafts.
...
Choose **Manage → Add your info → Import or paste…** to select statements, bank balances, crypto holdings, or metals for bulk entry. The individual asset cards keep the short guided forms.

Use **Update all balances** in Accounts or **Update all quantities** in Crypto to start from existing records. Unselected asset types stay out of the viewer unless they contain saved data. Manage → Tracking controls visibility without altering valuation history.
...
The top buttons keep management/import, privacy and lock together. Performance and Net worth sit in the top navigation row. The shared time, scope and status controls sit beneath them; detail pages put Back and the title above the time selector. A compact status button in the filter row opens missing-data details and Wise progress/retry actions without expanding the dashboard. Performance charts rescale for the selected entity and period. Selecting a chart month opens its monthly result. Company details show revenue, costs, profit and ownership share. Provisional or partial results retain a short headline label. Charts use sparse round-number axes, restrained smoothing, a light fill and exact values on hover; missing observations remain gaps. Secondary actions and menus use native bordered controls throughout the app. Full-card choices have one visible outline and pressed/disabled feedback. Entry classification still matters: owner-paid expenses already counted in company accounting and proceeds from asset sales need review if imported as personal spending or income.
