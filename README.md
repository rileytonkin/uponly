...
- Two tabs: **Net worth** and **Cash flow**. Net worth is always today's value, in dollars and cents, with an eye button beside it to hide values. Under it, two lines that don't change with the chart: **24h**, today's total against 24 hours ago (shown only when the same accounts and portfolios were counted then and every price is within three hours of each moment), and **All-time**, profit against what you paid for holdings with recorded purchases (“on $4,200 paid”, noting how many holdings that covers). A segmented control above the chart picks **1W 1M 3M 1Y All**; the chart's caption gives the change over that range, the line turns green when the range ends at or above where it started and red when below, values sit on the right axis, and the x-axis names only the first and last dates. On All assets, an allocation bar splits the total into banks, crypto and gold & silver, and every row shows its own change over the range. Cash flow keeps a By month, By year or All time selector and can show All, Personal, or a connected company.
