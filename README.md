...
Up Only starts empty. Two short setup steps save your recovery code and offer automatic reference exchange rates and metal prices. Finish with **Add your first balance** or **Skip for now**. Everything else is added from the **+** button on the home screen: a bank balance, crypto, gold and silver, a single income or expense, or a bank statement CSV. No account registration, analytics, connected wallets or cloud sync.

## What it does

- One title at the top says what you're looking at: **All assets**, a bank group, a portfolio, a company or **Income & spending**. The eye beside the title hides values on every page (⇧⌘P). Click the ⌃⌄ box (or press ⌘K) to switch: the list shows each one's value, its change over the chart's range and its share of your total (a part-owned company counts at your share), as one list in the same style as the home rows, with a check on the one showing. Under it, a small donut breaks your total down into bank balances, crypto, gold & silver and companies (hidden in privacy mode). Adding and managing stay with the + and … beside the title. Esc closes it. There are no tabs or Back bars on the dashboard; the switcher is the way around.
- Under the title: the value in dollars and cents, then the change over the chart's range as a green or red pill with what it's measured against ("↑ 21.0% vs $10,000.00 prev 30D", measured from the first saved value when the history is shorter than the range; hover for the dates), and **All-time** (profit against what you paid, for holdings with recorded purchases). A **24H 7D 30D 12M All** control sits above the chart and sets every change on the page; the line is green when the range ends at or above where it started and red when below. On All assets, each row shows its value and its change over the range and opens its page. In privacy mode amounts become dots but percentages stay, since they don't say how much you hold. Income & spending keeps its By month, By year or All time selector.
- Logos for the 250 largest coins by market cap (from CoinGecko, September 2026) ship inside the app as tiny images, so none is fetched and no server learns what you hold; other coins show a coloured letter.
- Hovering a chart shows the day, its value and, on net worth charts, the change since the chart's first day; the line after the cursor fades back. In privacy mode the axis and hover show dots for amounts and keep percentages.
- Every dashboard page opens at the height of All assets, which never scrolls; a longer page, such as a portfolio with many coins, scrolls within that height.
- Charts: 24H is valued hour by hour, as the app would have shown each hour from the prices saved through the day; the other ranges draw one point a day (All keeps to about a year's worth). The value axis is on the left, the line ends at the figure above it, and the time axis marks each day (7D), Monday (30D), month (12M, with January as its year) or year (a long All), or every six hours on the Mac's clock (24H), with a faint line at each. When a saved day lacks something, the chart estimates it from the nearest saved values and the hover says what: a coin's price or a currency's rate (a saved one within three days, else a straight line between the saved ones either side, else the nearest within 90 days), an account's balance before its first one (its first balance, within 90 days), or a company's ownership share for a month before its first recorded share (that first share). A day that still can't be valued is left out rather than drawn low; the line runs across up to a week of them and leaves a longer gap as a gap. The saved daily values themselves are never changed. Daily values are stored for All assets and each portfolio.
...
- When something is missing, a **Needs attention** row appears above the headline and names the item, or the count when there are several (for example “Balance needed for Everyday”). It opens missing balances, quantities, prices, accounting and month-completion checks. The current month is never flagged; once a month ends, the row asks whether it is complete. A month with no transactions can be confirmed with **Nothing to record this month**. A balance or partial import never confirms complete spending; closed months require an explicit review, and new accounts or changed entries reopen the relevant reviews.
- Compact bank/profile rows show one full USD balance, alongside crypto and metals. A portfolio page lists its holdings as a market app does — asset (its logo and ticker, with the quantity underneath), price with its move over the range, and value — sortable by value, change or name, with the full name and what you paid on hover and **Update holdings** underneath. Metals are always priced per troy ounce. Privacy mode hides what you hold but keeps prices and moves, which are public, and doesn't sort by value. Hourly prices are kept for 48 hours; older days keep one price each. Company pages show bank cash, your share, period profit/loss, history and underlying accounts.
...
Personal Performance describes recorded income less personal spending, including what a connected company paid you. All swaps those company payments for your ownership-weighted share of company profit from connected accounting, so nothing is counted twice; a company scope shows its full profit and your share separately. Missing accounting and personal records produce gaps or explicitly partial period totals. These are not investment returns. Net-worth changes include deposits, withdrawals and market moves. Missing observations remain missing in saved history; only the charts estimate a missing price from the nearest saved day, and say so. A month stays provisional until reviewed. Bank balances need manual updates; importing transactions does not infer a closing balance.
...
For a synthetic preview and tests, use **UpOnlyFixture**. It uses a temporary vault, in-memory keys, and fabricated figures; it never opens the production vault. Fixture data is excluded from the production build. Set `UPONLY_PREVIEW_DESTINATION=welcome`, `recovery` (save a new code), `locked`, `recover` (enter a saved code), `setup`, `sources`, `networth`, `tracking`, `preferences`, `security`, `metals`, or `import` when launching the fixture. Use `UPONLY_PREVIEW_TRACKED=banks`, `crypto`, `metals`, `cashFlow`, or comma-separated combinations. `UPONLY_PREVIEW_DARK=1` selects dark appearance; `UPONLY_PREVIEW_CAPTURE=1` writes a synthetic screenshot and prints its path. Use `UPONLY_PREVIEW_LONG_TEXT=1` to exercise wrapping with long names, filenames, and large amounts. `UPONLY_PREVIEW_MANY_COINS=1` adds more coins whose saved prices have gaps, to check the charts' estimates. With `networth-companies`, `UPONLY_PREVIEW_OWNERSHIP_GAP=1` records a company's share only from this month. All management previews use the same 344-point panel as the app, with a borderless fixture host.
...
Known Monzo (including the full export with Money In/Out columns), Wise (`dd-MM-yyyy` dates) and Up Only statement headers are recognized. Wise's newer transaction history export is read too: only completed transactions are imported, money out includes a fee charged in its currency, a conversion between your balances becomes a transfer out of one and into the other, and a file with several currencies becomes one statement per currency, each proposing a “Wise · EUR”-style account. Comma, semicolon and tab delimiters are detected, and UTF-8, UTF-16 and Windows-1252 text is read. Other exports have editable column mapping, a date format detected from every row (with dotted, dashed, unpadded and date-plus-time variants), a number format, and optional headers. Amounts may use `+12.34`, `(12.34)`, `12.34-`, a Unicode minus, and space or apostrophe thousands separators. Use one account per statement file or pasted table; create accounts inline without an opening balance if needed. Statement imports never infer closing balances.
...
Statement mapping also supports separate Debit/Credit columns. A Type column is used only when every value is `income`, `expense`, `refund` or `transfer`; income, expense and refund amounts are then nonnegative, while a transfer takes its direction from its sign (`-` out, `+` in; an unsigned transfer is left out of rebuilt balance history). Otherwise the amount sign determines the classification, re-checked whenever the number format changes. A refund reduces spending instead of counting as income; Monzo exports file merchant refunds and cashback as refunds automatically. Headerless statement paste starts with Date, Description, Amount, Currency, Type, TransactionID.
