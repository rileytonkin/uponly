...
- One title at the top says what you're looking at: **All assets**, a bank group, a portfolio, a company or **Income & spending**. Click the ⌃⌄ box beside it (or press ⌘K) to switch: the list shows each one's value, its 24-hour move and its share of the whole, with **Manage** and **New** at the bottom. There are no tabs or Back bars on the dashboard; the switcher is the way around.
- Under the title: the value in dollars and cents with an eye button to hide values, then **24h** (today against 24 hours ago, shown only when the same accounts and portfolios were counted then and every price is within three hours of each moment) and **All-time** (profit against what you paid, for holdings with recorded purchases). A **1W 1M 3M 1Y All** control sits above the chart; the line is green when the range ends at or above where it started and red when below. On All assets, each row shows its value and its 24-hour move and opens its page. In privacy mode amounts become dots but percentages stay, since they don't say how much you hold. Income & spending keeps its By month, By year or All time selector.
...
- Compact bank/profile rows show one full USD balance, alongside crypto and metals. A portfolio page shows its holdings as a table — asset (a coloured letter badge; no logos are fetched), price with its 24-hour move, and value with the quantity — sortable by value, 24-hour move or name. Metals are priced per troy ounce from one ounce up, otherwise per gram. Privacy mode hides what you hold but keeps prices and moves, which are public. Hourly prices are kept for 48 hours so the 24-hour figures are real; older days keep one price each. Company pages show bank cash, your share, period profit/loss, history and underlying accounts.
