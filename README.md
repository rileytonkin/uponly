...
- Two tabs: **Net worth** and **Cash flow**. Net worth is always today's value. Its headline shows the change over the chosen range in green or red, with a percentage (for example “+$4,599 (+50.5%) · past year”, or “since Jul 3” when history starts later). Range chips under the chart — **1M 3M 1Y 2Y 5Y** — pick how much history to show; each is a rolling window. On All assets, an allocation bar splits the total into banks, crypto and gold & silver, and every row shows its own change over the range. Cash flow keeps a By month, By year or All time selector and can show All, Personal, or a connected company.
...
- Compact bank/profile rows show one full USD balance, alongside crypto and metals. A portfolio page lists each holding with its quantity and unit price (“0.1 BTC · $59,000.00”; metals in troy ounces from one ounce up, otherwise grams). Company pages show bank cash, your share, period profit/loss, history and underlying accounts.
