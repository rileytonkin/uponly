...
- Charts: when a saved day lacks a coin's price or a currency's rate, the chart uses the nearest saved one within 30 days and the hover names it ("Estimated with Bitcoin at its Sep 10 price"); a day that still can't be valued in full is left out rather than drawn low, and the line runs across it. The saved daily values themselves are never changed.
...
For a synthetic preview and tests, use **UpOnlyFixture**. It uses a temporary vault, in-memory keys, and fabricated figures; it never opens the production vault. Fixture data is excluded from the production build. Set `UPONLY_PREVIEW_DESTINATION=welcome`, `recovery` (save a new code), `locked`, `recover` (enter a saved code), `setup`, `sources`, `networth`, `tracking`, `preferences`, `security`, `metals`, or `import` when launching the fixture. Use `UPONLY_PREVIEW_TRACKED=banks`, `crypto`, `metals`, `cashFlow`, or comma-separated combinations. `UPONLY_PREVIEW_DARK=1` selects dark appearance; `UPONLY_PREVIEW_CAPTURE=1` writes a synthetic screenshot and prints its path. Use `UPONLY_PREVIEW_LONG_TEXT=1` to exercise wrapping with long names, filenames, and large amounts. `UPONLY_PREVIEW_MANY_COINS=1` adds more coins whose saved prices have gaps, to check the charts' estimates. All management previews use the same 344-point panel as the app, with a borderless fixture host.
