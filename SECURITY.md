...
The providers see requests and network metadata, including IP addresses. CoinGecko sees, for current prices, only a request for the 250 (or 500) largest coins, unless a coin outside those is held; history requests name the coin. Binance sees the trading pair and date range of older history it's asked for (for gold, PAXG). Swissquote sees a metal symbol. Wise, in the private build, also sees currency codes and date ranges for rates. Frankfurter sees currency codes and historical date ranges. Gold API sees metal symbols (and date ranges, only if a history key was saved by an earlier version). No provider receives quantities, bank balances, account names, portfolio names, statements or vault keys.
