...
The providers see requests and network metadata, including IP addresses. CoinGecko sees its API key and, for current prices, only a request for the 250 (or 500) largest coins, unless a coin outside those is held; history requests name the coin. Binance sees the trading pair and date range of older history it's asked for. Swissquote sees a metal symbol. Wise, in the private build, also sees currency codes and date ranges for rates. Frankfurter sees currency codes and historical date ranges. Gold API sees metal symbols, date ranges and an optional history API key. No provider receives quantities, bank balances, account names, portfolio names, statements or vault keys.
