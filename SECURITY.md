...
Financial records, portfolio names, quantities, bank balances, API settings, imported statements and historical observations are encrypted together using CryptoKit AES-256-GCM. Authentication covers the vault ID and generation. Saves retain the previous encrypted generation (a file clone, size-checked when written and authenticated when opened) and use atomic replacement under a process lock, flushed to the drive with `F_FULLFSYNC`. If the current vault file can't be read or authenticated, unlock and recovery open the previous generation instead, save it back as current and tell you that the most recent change may be missing.
...
**Create a new recovery code…** in Backup & security wraps the same vault key with a new random code after Touch ID or the Mac password, then replaces the recovery wrapper atomically (temporary file, `F_FULLFSYNC`, replace). The old wrapper is not kept, so the old code no longer opens this vault. If the write fails or authentication is cancelled, the old wrapper and code stay in effect. The vault key does not change: backups exported earlier, and OS backups of the app’s data, still contain the old wrapper and still open with the old code.
