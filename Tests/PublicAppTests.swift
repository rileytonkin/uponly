...
struct BulkInputTests {
    ...
    func monthEvidence() {
        ...
        doc.entries = [
            Entry(month: august, kind: .income, amount: 4371.14, currency: "GBP", label: "Northwind", source: .csv, sourceRef: monzo.id.uuidString + ":1"),
        ...
        #expect(evidence.sources.map(\.name) == ["Monzo", "Alex", "Added by hand"])
        ...
        #expect(evidence.largestIncome.map(\.label) == ["Northwind"] && evidence.largestSpending.map(\.label) == ["Airbnb", "Cafe", "Cash"])
